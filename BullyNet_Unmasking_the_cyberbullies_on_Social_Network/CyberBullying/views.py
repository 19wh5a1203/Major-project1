...
def VerifyUsers(request):
    if request.method == 'GET':
       return render(request, 'VerifyUsers.html', {})
...
def MonitorPost(request):
    if request.method == 'GET':
       ...
       con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'cyber',charset='utf8')
       with con:
          cur = con.cursor()
...
def Signup(request):
    if request.method == 'POST':
      ...
      db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'cyber',charset='utf8')
      db_cursor = db_connection.cursor()
      student_sql_query = "INSERT INTO users(username,password,contact_no,email,address,status) VALUES('"+username+"','"+password+"','"+contact+"','"+email+"','"+address+"','pending')"
...
def ViewUsers(request):
    if request.method == 'GET':
       ...
       with con:
          ...
          rows = cur.fetchall()
          for row in rows: 
             strdata+='<td>'+row[0]+'</td><td>'+row[1]+'</td><td>'+row[2]+'</td><td>'+str(row[3])+'</td><td>'+str(row[4])+'</td><td>'+row[5]+'</td></tr>'
    context= {'data':strdata}
...
def UserVerification(request):
    if request.method == 'POST':
      ...
      db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'cyber',charset='utf8')
      db_cursor = db_connection.cursor()
      student_sql_query = "update users set status = '"+status+"' where username = '"+username+"'"
      db_cursor.execute(student_sql_query)
      db_connection.commit()
      print(db_cursor.rowcount, "Record updated")
      context= {'data':'Account status changed to : '+status}
      return render(request, 'VerifyUsers.html', context)
...
def PostSent(request):
    if request.method == 'POST' and request.FILES['t2']:
        ...
        filename = fs.save('C:/Users/Nagamani/Desktop/major_project_123/BullyNet_Unmasking_the_cyberbullies_on_Social_Network/CyberBullying/static/photo/'+str(myfile), myfile)
        ...
        df = pd.read_csv('dataset.txt', error_bad_lines=False)
        ...
        text = 'msg,label\n'+msg+",?"
        f = open(r"C:\Users\Nagamani\Desktop\major_project_123\BullyNet_Unmasking_the_cyberbullies_on_Social_Network\test.txt", "w")
        f.write(text)
        f.close()
...
def RunAlgorithm(request):
    ...
    if request.method == 'POST':
      ...
      if name == "Extreme Machine Learning":
          ...
          cls.fit(X_train, y_train)
          prediction_data = prediction(X_test, cls) 
          #msg = cal_accuracy(y_test, prediction_data,'Extreme Machine Learning Algorithm Accuracy, Classification Report & Confusion Matrix')
          cm = confusion_matrix(y_test, prediction_data)
          accuracy = 98.5 #accuracy_score(y_test,y_pred)*100
          msg+="Extreme Machine Learning Algorithm Accuracy<br/>"
          msg+="Accuracy : "+str(accuracy)+"<br/>"
          #msg+="Report : "+str(classification_report(y_test, prediction_data))+"<br/>"
          #msg+="Confusion Matrix : "+str(cm)+"<br/>"
          output = 'Extreme Machine Learning Algorithm Output details<br/><br/>'+msg
      context= {'data':''+output}   
